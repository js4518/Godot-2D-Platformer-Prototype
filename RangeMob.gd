...
func _physics_process(delta):
			...
			if not $"../Character".dead:
				...
				if $close_move_timer.is_stopped():
					if $"../Character".position.x > position.x: 
						if ($"../Character".position.x - position.x > 250) and ($"../Character".position.x - position.x < 300): velocity.x = 0
						elif $"../Character".position.x - position.x < 250: velocity.x = -speed
						else: velocity.x = speed
					elif $"../Character".position.x < position.x:
						if (position.x - $"../Character".position.x > 250) and (position.x - $"../Character".position.x < 300): velocity.x = 0
						elif position.x - $"../Character".position.x < 250: velocity.x = speed
						else: velocity.x = -speed
	...
	if state_detect or state_attacked:
		if $"../Character".position.x > position.x: 
			if ($"../Character".position.x - position.x > 250) and ($"../Character".position.x - position.x < 300): $AnimatedSprite.animation = "right_detect"
			elif $"../Character".position.x - position.x < 250: $AnimatedSprite.animation = "left_detect"
			else: $AnimatedSprite.animation = "right_detect"
		elif $"../Character".position.x < position.x:
			if (position.x - $"../Character".position.x > 250) and (position.x - $"../Character".position.x < 300): $AnimatedSprite.animation = "left_detect"
			elif position.x - $"../Character".position.x < 250: $AnimatedSprite.animation = "right_detect"
			else: $AnimatedSprite.animation = "left_detect"
	else:
		if velocity.x >= 0:
